...
if [ "$DO_CHECK" = true ]; then
  REMOTE_VER=$(curl -sf --max-time 3 "https://raw.githubusercontent.com/skylavis-sky/plugin-store/main/skills/pancakeswap-v2-plugin/plugin.yaml" | grep '^version' | head -1 | tr -d '"' | awk '{print $2}')
...
if [ "$REMOTE_VER" != "$LOCAL_VER" ]; then
  ...
  npx skills add skylavis-sky/plugin-store --skill pancakeswap-v2-plugin --yes --global 2>/dev/null || true
...
npx skills add skylavis-sky/plugin-store --skill plugin-store --yes --global
```
...
if [ ! -f "$LAUNCHER" ]; then
  ...
  curl -fsSL "https://raw.githubusercontent.com/skylavis-sky/plugin-store/main/scripts/launcher.sh" -o "$LAUNCHER" 2>/dev/null || true
  chmod +x "$LAUNCHER"
fi
if [ ! -f "$CHECKER" ]; then
  curl -fsSL "https://raw.githubusercontent.com/skylavis-sky/plugin-store/main/scripts/update-checker.py" -o "$CHECKER" 2>/dev/null || true
fi
...
BIN_TMP=$(mktemp -d)
RELEASE_BASE="https://github.com/skylavis-sky/plugin-store/releases/download/plugins/pancakeswap-v2-plugin@0.2.6"
...
curl -fsSL "${RELEASE_BASE}/checksums.txt" -o "$BIN_TMP/checksums.txt" || {
  echo "ERROR: failed to download checksums.txt for pancakeswap-v2-plugin@0.2.6" >&2
  rm -rf "$BIN_TMP"; exit 1; }
...
echo "0.2.6" > "$HOME/.plugin-store/managed/pancakeswap-v2-plugin"
```
